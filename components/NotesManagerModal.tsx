...
import { X, NotebookText, Search, BookOpen } from 'lucide-react';
...
const NotesManagerModal: React.FC<Props> = ({ isOpen, onClose, items, onSelectItem }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(item =>
      item.query.toLowerCase().includes(lowercasedTerm) ||
      (item.note && item.note.toLowerCase().includes(lowercasedTerm))
    );
  }, [items, searchTerm]);
        ...
        <div className="p-4 border-b border-gray-200 dark:border-slate-800 flex-shrink-0">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Tìm theo tên phác đồ hoặc nội dung ghi chú..."
          ...
          {filteredItems.length > 0 ? (
            <ul className="space-y-3">
              {filteredItems.map(item => (
                <li 
                  key={item.query} 
                  className="group flex flex-col gap-2 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors border border-gray-100 dark:border-slate-800"
                >
                  <div className="flex items-start justify-between">
                    ...
                    >
                      <p className="font-semibold text-gray-800 dark:text-slate-200 group-hover:text-medical-600 dark:group-hover:text-medical-400">
                        <Highlight text={item.query} highlight={searchTerm} />
                      </p>
                      <span className="text-xs text-gray-400 dark:text-slate-500">
                        Ghi chú ngày: {new Date(item.timestamp).toLocaleDateString('vi-VN')}
                      </span>
                    </div>
                    <button 
                      onClick={() => onSelectItem(item.query)}
                      className="p-2 text-gray-400 hover:text-medical-500 hover:bg-medical-50 dark:hover:bg-medical-900/30 rounded-full"
                      title={`Xem phác đồ & ghi chú của "${item.query}"`}
                    >
                      <BookOpen className="w-4 h-4" />
                    </button>
                  </div>
                  {item.note && (
                    <div className="text-sm text-gray-600 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 p-2 rounded-md border-l-2 border-amber-400 dark:border-amber-500 max-h-24 overflow-y-auto">
                      <Highlight text={item.note} highlight={searchTerm} className="whitespace-pre-wrap line-clamp-4" />
                    </div>
                  )}
                </li>
              ))}
            ...
            <div className="text-center py-16 text-gray-500 dark:text-slate-500">
              <p>{searchTerm ? 'Không tìm thấy ghi chú nào phù hợp.' : 'Bạn chưa có ghi chú nào.'}</p>
            </div>
          )}
        </div>
      </div>
...
export default NotesManagerModal;
